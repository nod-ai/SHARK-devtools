...
def parse_arguments():
    ...
    checkout_parser.add_argument(
        "--sync", action="store_true", help="Sync deps as repositories are checked out"
    )
    checkout_parser.add_argument(
        "--no-submodules", action="store_true", help="Disables all submodule updates"
    )
    checkout_parser.add_argument(
        "--no-deps", action="store_true", help="Disables checkout of dependencies"
    )
    checkout_parser.add_argument(
        "--ro",
        action="store_true",
    ...
    # 'pin' sub-command
    pin_parser = subparsers.add_parser("pin", help="Pin deps to current revisions")
    ...
    show_pins = subparsers.add_parser("show-pins", help="Show current dependency pins")

    # 'sync' sub-command
    sync_parser = subparsers.add_parser(
        "sync",
        help="Sync all dependent repositories to pinned deps of the current repository",
    )
...
def do_roll(args):
    ...
    try:
        ...
        raise types.CLIError(
            f"Unknown schedule '{schedule_name}' for {r.name}. "
