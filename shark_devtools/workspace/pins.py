    ...
    *,
    ...
    def callback(pinned_versions, origins, submodules) -> bool:
        ...
        for dep_name in repo.deps:
            ...
            summary = update_dep(
                ws, pinned_versions, repo, dep_repo, require_upstream=require_upstream
            )
            print(f"  {summary}")
...
def show(ws: types.WorkspaceMeta, repo: types.RepoInfo, repo_top: Path):
    ...
    for pin_repo_name, pin_revision in pins.items():
        ...
        git.fetch(pin_dir)
        cp = git.run(
            ["show", "--pretty=%C(auto)%h %s (%cd)", pin_revision], pin_dir, silent=True
        )
...
def process_pin_file(repo_top: Path, callback) -> bool:
    ...
    if callback(
        pinned_versions=pinned_versions, origins=origins, submodules=submodules
    ):
        # Write new one.
